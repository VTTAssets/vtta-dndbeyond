        ...
        switch (message.type) {
          case 'monster':
            ...
            result.world.entities = game.actors.entities
              .filter(
                entity => entity.data.type === 'npc' && entity.data.name === message.name && entity.owner === true
              )
              .map(entity => {
                return {
                  id: entity.id,
              ...
              result.scene.entities = game.scenes.active.data.tokens
                .map(token => {
                  let actor = game.actors.entities.find(
                    actor => actor.id === token.actorId && actor.name === message.name && actor.owner === true
                  );
            ...
            if (compendiumName !== '') {
              ...
              if (compendium) {
                compendium
                  .getIndex()
                  .then(index => {
                    let entity = index.find(entity => entity.name === message.name);
                    if (entity) {
            ...
            if (compendiumName !== '') {
              ...
              if (compendium) {
                compendium
                  .getIndex()
                  .then(index => {
                    let entity = index.find(entity => entity.name.toLowerCase() === message.name.toLowerCase());
                    if (entity) {
            ...
            if (compendiumName !== '') {
              ...
              if (compendium) {
                compendium
                  ...
                  .catch(error => {
                    utils.log(`Error searching through compendium ${compendiumName}`, 'extension');
                ...
                compendium
                  ...
                  .then(async index => {
                    ...
                    for (let i = 0; i < message.name.length; i++) {
                      let entity = index.find(entity => entity.name.toLowerCase() === message.name[i].toLowerCase());
                      if (entity) {
                        // get the spell and deliver it back to the query
