  ...
  activateListeners(html) {
    $(html)
      ...
      .on('paste', async event => {
        ...
        let items = [result.actions, result.inventory, result.spells, result.features, result.classes].flat();
        utils.log('Character items', 'character');
        utils.log(items, 'character');

        let itemImportResult = await this.actor.createManyEmbeddedEntities('OwnedItem', items, {
          displaySheet: false,
        });
        ...
        if (game.user.isGM && importPolicy !== 2) {
          ...
          for (let category of [
            ...
            let compendiumLabel = game.settings.get('vtta-dndbeyond', category.compendium);
            ...
            if (compendium) {
              ...
              for (let i = 0; i < result[category.type].length; i++) {
                ...
                if (searchResult && importPolicy === 0) {
                  showCurrentTask(`Updating item ${item.name} in compendium ${compendiumLabel}`);
                ...
                if (!searchResult) {
                  ...
                  showCurrentTask(`Creationg item ${item.name} in compendium ${compendiumLabel}`);
